...
- **2026-07-07** — **F5 implementované (D-055, D-050, dodatok D-044):** 3D vrstva na **WebGPU** `@ifc-lite/renderer` + parser/geometry/spatial; federácia N modelov (`federationRegistry`, per-model visibility); IFClite-native navigátor (SPATIAL/TYPE/MATERIAL/CLASS, follow-IFClite — nerecyklovaný `fetchSpatialTree`); toolbar (rezy/pohľady/reset); aktivovaný Query (`IfcQuery`+DuckDB); pick → GUID → DB karta, inak IFC-props panel. N-ary `ifc_upload.py` + `getIfcModels()`. Build fix: `laz-perf` alias (point clouds nepoužívame). `tsc`/`lint`/build-compile ✅; **runtime WebGPU test + VZT upload ostáva.** Vetva `cursor/f5-ifclite-webgpu`.
