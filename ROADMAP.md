...
- **2026-07-19** — **Experiment RoomPlan → IFC4 dodatok (D-078):** opening voidy (dvere/okná režú stenu), polyline `IfcSpace` footprint (L-shape 6 rohov), a ETL dry-run proof (`etl.main --dry-run` stageuje 14 objektov, otvory vylúčené D-034) — logy commitnuté. Stále experiment mimo produkčného modelu; vetva `claude/lidar-to-ifc-converter-8d7p1v`.
- **2026-07-18** — **Experiment: RoomPlan → IFC4 (D-078):** izolovaný PoC `experiments/roomplan-to-ifc/` — CapturedRoom JSON → IFC4 konvertor (ifcopenshell), 3 syntetické fixtures, roundtrip testy + web-ifc smoke check PASS; netestovaný SwiftUI LiDAR scanner scaffold. Mimo produkčného modelu; vetva `claude/lidar-to-ifc-converter-8d7p1v`.
