...
| **S-LLM → F6** LLM interface nad grafom | 🟢 **kritická cesta**, kadencia 1 hotová | D-047/D-056; jadro implementované: provider vrstva `lib/llm/` (Anthropic/mock, API-pluggable), read-only tools nad whitelist views, slučka `/api/ask`, UI `/ask` s trust-loop deep-linkami. **Čaká na `ANTHROPIC_API_KEY`** (Vercel env) na live beh; ventilový dotaz čaká na import ÚK/ZTI |
