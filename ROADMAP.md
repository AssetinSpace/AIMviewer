...
| **F9** Projektová dokumentácia v jednom rozhraní | 🟢 M1–M3 hotové na vetve | D-075; prepínač 3D\|2D\|Split + documents panel s kartami vo forku, bridge `DOCUMENTS_LOAD`; vetva `claude/pdf-rendering-document-view-v6nivv` (oba repá), čaká na review/merge + live overenie; M4 (doc↔doc split, per-tab pop-out) neskôr |
| **F10** AIM inspector — viewer-first konsolidácia | 🟢 hotové na vetve | D-077; audit duplicít + rešerš CDE → AIM \| IFC taby v PropertiesPanel (AimPanelData v2: zodpovednosti/captures/história), per-GUID badge dekorácie stromu (`AIM_TREE_DECORATIONS`), sidebar sa na `/ifc` nerenderuje, jednotný `AimPanelView` render 2D/3D, `filter-bar.tsx` zmazaný; vetva `claude/aim-viewer-ifc-consolidation-r7z04q` (oba repá), deploy fork-first; čaká na review/merge + live overenie proti nasadenému forku |
...
| **F9 — Projektová dokumentácia v jednom rozhraní** 🟢 M1–M3 hotové na vetve | Dalux-like workspace vo forku: prvotriedny prepínač **3D \| 2D \| Split** v toolbare (derivovaný režim nad D-072 flagmi + `underlayPlanFull`, 2D = zovšeobecnený `DrawingPlanPane` na celú plochu, fallback bez výkresu = locked ortho model), **documents panel + karty dokumentov** v resizable center pane (textové PDF virtualizovane cez `rasterizePdfPage`, obrázky; drag&drop lokálnych súborov, `local:` vzor), bridge `DOCUMENTS_LOAD`/`DOCUMENT_OPEN`/`DOCUMENT_EVENT` (súrodenec `UNDERLAYS_LOAD`), AIMviewer adapter `fetchProjectDocuments()` (`objects`/`documents` + `rel_associates_document` → `folder`); `/drawing/[id]` ostáva ako standalone flow. **Hotové na vetve `claude/pdf-rendering-document-view-v6nivv` (oba repá): M1** (`ViewModeSwitcher` v Main/MobileToolbar, `useViewMode`, `enterPlanView`, kolabovateľný `viewport-3d-panel`, výber výkresu pri >1 na podlažie), **M2** (`documentsSlice` + testy 6/6, panel `documents`, `DocumentsPanel`, `DocumentPane` napravo od 3D, `PdfDocumentView` ≤2400 px LRU ≤4, `ImageDocumentView`, mobil overlay), **M3** (bridge správy + testy 27/27, `fetchProjectDocuments()`, push po MODELS_LOADED, `?doc=` deep link). Overené: fork `pnpm typecheck` (34/34) + slice/bridge testy; AIM `tsc` čistý + vitest 32/32. **Ostáva:** review/merge, live overenie proti nasadenému forku, M4 (doc↔doc split, per-tab OS pop-out, `VIEW_SET`). | D-075 | stavia na D-072 (F7); M3 potrebuje E3 (documents); nezávislé od F2/F6 |
...
- **2026-08-02** — **F9: záložka „Documents" v ribbone forku (D-075 dodatok):** upstream spravil tabbed ribbon defaultným toolbarom, takže naše funkcie (documents, underlays, prepínač 3D\|2D\|Split + podlažie) ostali v defaultnom UI len na ikonovej lište vpravo — pridaná fork-only záložka **Documents** (skupiny Library / View mode), tlačidlá volajú tie isté store akcie, klasický toolbar si všetko ponecháva (nula zmien správania). Telo v izolovanej vrstve `aim/DocumentsRibbonTab.tsx`, zapojenie 3 riadky v `AIM-FORK` sentineloch, `docs/FORK_MAINTENANCE.md` tabuľka doplnená (9 → 15 riadkov: 4 chýbajúce súbory + 2 nové touchpointy). Overené: fork typecheck 32/32, viewer testy 1915 pass (+2 nové), živý preklik v Chromium (záložka, oba panely, 2D + storey picker, klasický toolbar bez regresie), CI PR #42 zelené. Zmergované ako fork PR #42; AIMviewer bez zmien kódu.
- **2026-08-01** — **Upstream sync IFClite stabilizovaný (D-071 dodatok):** audit ukázal fork **146 commitov (~3 týždne) pozadu** — cron čítal workflow z `main`, kam sa oprava z 27. 7. nikdy nedostala; beh 13. 7. hlásil „success" bez toho, aby otvoril PR (403 zožraté cez `|| echo`); zelený PR #35 päť dní nikto nezmergoval. Vo forku (PR #36): žiadne tiché zlyhania, bezkonfliktný sync sa po CI merguje sám (merge commit, nie squash), každý beh prepíše pinnutý issue `Upstream sync status` (koľko pozadu, výsledok behu, expirácia PATu) — týždenná kontrola jedného miesta. `FORK_MAINTENANCE.md` tabuľka touchpointov pregenerovaná: 3 → 9 súborov. Naživo: #36 + #35 zmergované → zaostávanie **146 → 27**, ostrý beh otvoril #37 (27 commitov, konflikty) — cyklus po prvý raz dobehol; status krok zhorel na tom, že forky majú vypnuté Issues, opravené v #38 (status vždy do job summary, issue best-effort). Ostáva klik v Settings: zapnúť Issues + variable `UPSTREAM_SYNC_TOKEN_EXPIRES`. V AIMviewer `@ifc-lite/*` pinnuté na presnú verziu + `renovate.json` (pondelok, zoskupené, majory oddelene); `@ifc-lite/wasm` je 2 majory pozadu (2.13.4 vs 4.2.0), bump príde samostatným Renovate PR.
- **2026-07-17** — **F9: identifikátorové hyperlinky v 2D pláne (D-076):** kódy prvkov v texte PDF underlay sa rozpoznávajú konfigurovateľným regexom a klikajú na select + preview prvku (tie isté store akcie ako pick v scéne); zdroj kódu per projekt (Name/Description/ObjectType/Tag/Pset.property s fallbackom), index kód→GlobalId s cache, duplicity riešené podlažím výkresu + výberom kandidátov, settings s live-test regexu v underlay paneli. Genericky vo forku (vetva `claude/ifc-identifier-hyperlinks-jzd5p5`); nové testy 27/27, viewer testy 1768 pass.
- **2026-07-17** — **F9: auto-prepínanie režimu pri dokumentoch (D-075):** klik na kalibrovaný výkres = Split view podlažia (PDF karta cez sekundárnu ikonu); otvorenie karty v 2D režime auto-klesne do Split, na mobile zavrie split overlay — karta sa vždy ukáže. Regresie smoke 15/15, pinch 5/5, testy 1741 pass.
- **2026-07-17** — **F9: mobile pinch-zoom PDF čítačky (D-075):** pinch na mobile padal do browser page-zoomu (rozmazané, hlásené z iPhone) — čítačka aj image viewer teraz pinch zachytávajú (pointer events, `touch-action`, iOS gesture preventDefault) a ženú ním vlastný ostrý zoom s kotvou na midpoint; double-tap fit↔3×; tlačidlá/Ctrl+wheel kotvené. CDP touch test: pinch → 833 %, re-raster 363→2173 px; regresie smoke 15/15, zoom 10/10, text 7/7, testy 1741 pass.
- **2026-07-17** — **F9: textová vrstva PDF čítačky (D-075):** selekcia/kopírovanie textu ponad canvas (pdf.js `TextLayer`, spany v % + `--total-scale-factor` → zoom bez re-renderu, render len pre stránky v okne). Playwright: dvojklik aj drag selekcia, zarovnanie po zoome; smoke 15/15, zoom 10/10, viewer testy 1741 pass.
- **2026-07-17** — **F9: vektorový zoom PDF čítačky (D-075):** sharp-crop overlay — viditeľný výrez sa pri prekročení stropu base rastra re-renderuje z PDF vektorov v presnej device mierke (`rasterizePdfRegion`), ZOOM_MAX 16; ostré na ľubovoľnom zoome (Playwright: overlay density 1.0 pri ~9× aj ~14×, cleanup pri oddialení/opustení okna). Viewer testy 1741 pass, smoke 15/15.
- **2026-07-17** — **F9 fix: ostrý zoom PDF čítačky (D-075):** raster sleduje zoom až po 4096 px (ZOOM_MAX 10), oprava pod-rastrovania portrait strán (target podľa šírky s korekciou pomeru strán), LRU canvasov podľa pixel budgetu (~32 MP). Overené Playwright zoom testom (hustota ≥1.0 do ~4×, strop 4096 pri ~9×); viewer testy 1741 pass.
- **2026-07-16** — **F9 M1–M3 implementované (D-075):** fork — `ViewModeSwitcher` (3D|2D|Split + storey picker) v Main/MobileToolbar, `useViewMode` (derivovaný režim), `enterPlanView`/`underlayPlanFull` (2D na celú plochu, 3D panel kolabovaný bez remountu), výber výkresu pri >1 na podlažie; documents workspace — `documentsSlice` (testy 6/6), panel `documents` v registry/sidebar/mobile, `DocumentsPanel` (strom podľa foldera, filter, drag&drop PDF/obrázkov), `DocumentPane` karty napravo od 3D, `PdfDocumentView` (virtualizované strany, raster ≤2400 px, LRU ≤4, per-tab page/zoom v store), `ImageDocumentView`; bridge `DOCUMENTS_LOAD`/`DOCUMENT_OPEN`/`DOCUMENT_EVENT` (validátory + testy, spolu 27/27). AIMviewer — `lib/data/documents.ts` (`fetchProjectDocuments`, ISR tag `aim`), wiring `ifc-viewer`/`ifc-workspace`/`/ifc` + `?doc=` deep link. Fork typecheck 34/34, AIM tsc + vitest 32/32. Ostáva review/merge + live overenie; M4 neskôr.
- **2026-07-16** — **F9 kandidát: projektová dokumentácia v jednom rozhraní (D-075):** koncept z analýzy CDE prístupov (Dalux/Revizto/ACC/Procore) — prepínač 3D|2D|Split v toolbare ifc-lite, documents panel + karty dokumentov v center pane, bridge `DOCUMENTS_LOAD`/`DOCUMENT_OPEN`/`DOCUMENT_EVENT`, AIMviewer ako dátový adapter; `/drawing/[id]` ostáva. Fázy M1–M4; len dokumentácia, implementácia po diskusii.
...
- **2026-07-14** — **F8: Reality Capture (D-073):** modul fotiek + statických 360° panorám ukotvených 2D/3D/IfcSpace, obojsmerne. Model „ako dokumenty" (`captures`/`capture_media` prípony, ukotvenie v `_capture`, 2 prvé `aim_` hrany manifestu), migrácia `20260716120000`; úložisko Supabase bucket `captures` + `sharp`; browser upload za `CAPTURE_WRITE_ENABLED` (`/api/captures`); galéria na `/node/[id]` + capture pin na 2D vieweri; 360 = Photo Sphere Viewer host-side; 3D piny vo forku (reuse `annotationsSlice`/`AnnotationLayer`) + bridge; verzovanie append-only. Zatvára otvorené body D-065.
- **2026-07-12** — **F6: style_in_3d podľa vlastnosti psetu (dodatok D-066):** `property`+`value`(+`pset`) filter — psety zo slovníka `v_property_dictionary`, match nad `v_asset_effective` (dedičnosť z typu), case-insensitive presná zhoda; prompt zakazuje aproximovať vlastnosť triedami. Fix: „vyizoluj nosné (LoadBearing=true)" už neizoluje aj nenosné steny.
- **2026-07-12** — **F6: hlasový vstup pokynov (D-069):** mikrofón v ask-paneli — diktát (MediaRecorder webm/opus, Safari mp4) → `/api/transcribe` → prepis cez STT provider vrstvu `lib/stt/` (default Gemini audio `inline_data` na existujúcom `GEMINI_API_KEY`, free tier; `STT_PROVIDER=mock` pre testy) → editovateľný text v inpute, žiadne auto-send. Doménový slovník (VZT/ÚK/ZTI, kódy DD01.06.03) v prompte; rate limit + origin guard podľa D-068. Overené Playwright harnessom s fake mikrofónom.
- **2026-07-12** — **F6: AIM karta v paneli viewera (D-067):** detail vybraného prvku sa renderuje v natívnom paneli embed viewera (host schéma `lib/aim-panel.ts` → bridge `AIM_PANEL_DATA` → fork `AimCard.tsx`, navigácia späť cez `AIM_NAVIGATE`); `ifc-workspace.tsx` prepnutý na wrapper `ifc-viewer.tsx`, `ElementInfoPanel` overlay v 3D odstránený.
- **2026-07-12** — **F6: AI chat ovláda 3D scénu (D-066):** tool `style_in_3d` (ofarbenie/skrytie/izolácia/reset; výber celej triedy filtrom ifc_type/predefined_type, cap 400 GUIDov) → URL `ops` formát → nové AimBridge správy do IFClite forku (COLORIZE/HIDE/SHOW/ISOLATE/SHOW_ALL/RESET_COLORS → `bim.viewer.*`). Efekty sa v scéne hromadia, reset explicitný; vyžaduje redeploy forku (vetva aim-integration).
- **2026-07-10** — **F6: obsah dokumentov (D-063):** tabuľka `document_pages` (text PDF strán per strana, GIN FTS index) + `etl/pdf_text.py` (PyMuPDF, všetky PDF z docs.csv, idempotentný delete+insert, --dry-run) + RPC/tool `search_documents` (websearch FTS, ts_headline snippet, deep_link `/drawing/<id>?page=<n>`, dokumenty do trust-loop zdrojov). OCR mimo scope — text výkresov je riedky (legendy/pečiatky), poctivo v hinte toolu. Extrakcia beží tam, kde sú PDF lokálne (workflow ako E4). Migrácia `20260715120000`. Eval kategória documents_content.
- **2026-07-10** — **F6: procedúra výberu modelu (D-062):** eval-driven výber plateného modelu pre /api/ask (kandidáti: claude-sonnet-5 primárny, claude-haiku-4-5 dolná hranica, claude-opus-4-8 horná, platený Gemini; ciele: negative 100 %, psets_custom ≥ 75 %, celkovo ≥ 85 %). Spúšťa sa po `supabase db push` migrácií D-058–D-060 na prod + doplnení verified hodnôt v eval sade; samotná zmena = `LLM_MODEL`/`LLM_PROVIDER` env.
- **2026-07-10** — **F6: statický IFC slovník psetov (D-061):** `etl/pset_manifest.py` (zrkadlo manifest.py — PsetQto bSDD šablóny, `--sql`/`--check`/`--classes-from-db`, deterministický výstup) → referenčná tabuľka `ifc_property_definitions` (973 properties / 127 psetov pre 25 tried projektu: description, data_type, enum_values, applicable_classes) vo whiteliste query_view; prompt „význam → ifc_property_definitions, výskyt → v_property_dictionary". Migrácia `20260714120000`.
- **2026-07-10** — **F6: agregácie + numericky bezpečné filtre (D-060):** RPC `aggregate_objects` — sum/avg/min/max/count nad hodnotami psetov (guarded numeric cast, `skipped_non_numeric`), group_by stĺpec/pset cesta, AND filtre s numerickým gt/lt nad path (PostgREST porovnáva JSONB text — koreň chyby), `ids` reťazenie, `return_rows` escape hatch; tool + guidance guard v query_view + prompt „súčty počíta DB". Injection sada overená na lokálnom PG 16. Migrácia `20260713120000`. Eval kategória aggregation (4 otázky).
- **2026-07-10** — **F6: fulltext nad všetkým (D-059):** `objects.search_text` (generated stĺpec: unaccent+lower flattening identity a všetkých psetov vrátane custom) + GIN tsvector/pg_trgm indexy + RPC `search_everything` (FTS + fuzzy word_similarity 0.4, matched_properties = dôkaz kde match nastal) + tool `search_everything` a prompt (obsah vlastností → fulltext, citovať dôkaz). Custom psety prvýkrát vyhľadateľné — eval kategória psets_custom je metrika. Overené na lokálnom PG 16 (diakritika, preklep, custom pset, multiword). Migrácia `20260712120000`.
...
- **2026-07-10** — **F6: opakované „zobraz v 3D" v jednom chate (D-056 dodatok 3b, kolo 3):** identická focus URL bola router no-op a `staleTimes` cache mohla podržať starý render → každá 3D akcia nesie unikátny nonce `&r=` a viewer efekt beží na `[focus, focusNonce]`; staré zvýraznenie sa pri novej požiadavke obnoví na pôvodné materiály. Prompt: nová požiadavka = nový show_in_3d s cieľovou množinou („pridaj" = únia). Playwright replika mechaniky 4/4.
- **2026-07-10** — **F6: focus reaguje na soft navigáciu (D-056 dodatok 3b, root cause):** hlavný efekt viewera beží len na `[modelsKey]`, focus sa aplikoval iba pri načítaní modelu — AI dock mení `?focus=` soft navigáciou bez remountu, takže sa ignoroval. `applyFocus` v ref-e + efekt na `[focus]` (vzor `applyFloorFilterRef`), obnova materiálov predchádzajúceho focusu, multi-floor vypne floor filter.
