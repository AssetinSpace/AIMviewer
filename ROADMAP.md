...
| **F4** PDF prehliadačka rework | 🛠️ rozpracované | D-054; kadencia 1: interakcia vieweru (koliesko=zoom-to-pointer, pinch+pan, fullscreen, fit-to-width). Kadencia 2: double-tap zoom, CSS-preview gest (rerastr až po ustálení), mobilný bottom-sheet panela + 2 bugfixy z live verifikácie (mŕtvy desktop klik na región, ustrelená kotva zoomu). Ostáva: výkon prvého načítania veľkých PDF |
...
| **F4 — PDF prehliadačka rework** 🛠️ | Prestavaná prehliadačka výkresov/dokumentov (UX/výkon) — `/drawing/[id]` a spol. **Kadencia 1 hotová:** interakcia vieweru (`drawing-viewer.tsx`) — koliesko=zoom-to-pointer, pinch+drag-pan, fullscreen, fit-to-width; prekliky/`focus`/overlay zachované. **Kadencia 2 hotová:** double-tap zoom, výkon gest (CSS preview + rerastr po ustálení), mobilný bottom-sheet panela, fix mŕtveho desktop kliku na región a kotvy zoomu; overené live (Playwright). Ostáva: výkon prvého načítania veľkých PDF. | D-054 | nezávislé; skorý quick-win |
